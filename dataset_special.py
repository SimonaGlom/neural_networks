...
class Animal:
    ...
    def get_records(self, max_spectograms=0):
        if max_spectograms == 0:
...
if __name__ == '__main__':
    ...
    for i in range(1, 100):
        ...
        writer = csv.writer(open("src/experiments/e1/data/" + str(i) + ".csv", 'w'))
        writer.writerow(["src", "sk_name", "latin_name", "class_name"])

        with open("src/experiments/e1/data/" + str(i) + ".csv", 'w', newline='') as file:
            for animal in animals:
                ...
                if animal.get_records_count() > 1:
                    for record in animal.get_records(max_spectograms=10*i):
